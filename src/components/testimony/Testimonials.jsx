...
const Testimonials = () => {
  ...
  return (
    <>
      <div className="Testimonials h-full mx-4 md:mx-0 grid grid-cols-1 gap-4 md:grid md:grid-cols-3 md:justify-items-center md:align-items-center md:gap-4 md:px-28 py-8 ">
        {testimonialData.map((item) => (
          <div
            key={item.heading}
            className="shadow-lg rounded-lg p-2 bg-slate-50 dark:bg-darkCardTheme"
            data-Aos="zoom-in-up"
          >
            <p className="font-semibold text-lg text-lightFontHeading dark:text-lightFontText">{item.heading}</p>
            <p className="leading-loose text-lightFontHeading dark:text-darkFontText">{item.description}</p>
          </div>
        ))}
      </div>
