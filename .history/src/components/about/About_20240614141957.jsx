...
const About = () => {
  ...
  const scaleProgress = useTransform(scrollYProgress, [0, 1], [.9, 1]);
  const opacityProgress = useTransform(scrollYProgress, [0, 1], [0.9, 1]);
  return (
    <>
      <motion.div
        className="mx-4 h-full grid grid-cols-1 gap-8 md:mx-0 md:flex md:justify-between md:items-center md:px-28 md:py-12 "
        ref={ref}
        style={{
          scale: scaleProgress,
      ...
      >
        <motion.img src={about_image} alt="about-me-image" />
        <p className="about-text leading-loose text-stone-600">
      ...
      <div className="values mx-4 md:mx-0 grid grid-cols-1 gap-4 md:grid md:grid-cols-3 md:justify-items-center md:align-items-center md:gap-8 md:px-28">
        {cardData.map((item) => (
          <motion.div
            ...
            transition={{ delay: 0.2, duration: 0.5, type: "spring", stiffness:  }}
          >
            <img
              className="rounded"
